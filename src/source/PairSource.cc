...
FinalState DecayPair(double pairMass, double pairMomentum, double daughterMass) {
    ...
    const double leptonMomentum =
        std::sqrt(leptonEnergy * leptonEnergy - electron_mass_c2 * electron_mass_c2);
...
void CheckConservation(const FinalState& state, double parentMass) {
    ...
    if (std::abs(state.positronEnergy + state.electronEnergy + state.recoilEnergy -
                 parentMass) > 1e-6 * MeV ||
...
void AddVertex(G4Event* event, G4ParticleDefinition* recoil,
    ...
    // GDML places the LiF coating center at the world origin.
    auto* vertex = new G4PrimaryVertex(G4ThreeVector(0, 0, 0), 0.0);
    vertex->SetPrimary(new G4PrimaryParticle(
        G4Positron::Definition(), state.positron.x(), state.positron.y(),
        state.positron.z()));
    vertex->SetPrimary(new G4PrimaryParticle(
        G4Electron::Definition(), state.electron.x(), state.electron.y(),
        state.electron.z()));
    vertex->SetPrimary(new G4PrimaryParticle(
        recoil, state.recoil.x(), state.recoil.y(), state.recoil.z()));
...
PairSource::PairSource(double transitionEnergy)
    ...
    if (transitionEnergy_ <= 2.0 * electron_mass_c2) {
        throw std::invalid_argument("Transition energy must exceed two electron masses");
    }
}
...
void PairSource::Generate(G4Event* event) {
    auto* recoil = G4ParticleTable::GetParticleTable()->GetIonTable()->GetIon(4, 8);
    ...
    const double pairMomentum = TwoBodyMomentum(parentMass, daughterMass, pairMass);
