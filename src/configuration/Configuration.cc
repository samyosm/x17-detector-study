...
template <typename T> T Required(const toml::table &document, const char *key) {
  const auto value = document.at_path(key).value<T>();
  if (!value)
    throw std::runtime_error(std::string("Missing or invalid TOML value: ") +
                             key);
  return *value;
}
...
std::array<double, 3> Vector(const toml::table &document, const char *key) {
  ...
  if (!values || values->size() != 3) {
    throw std::runtime_error(std::string("Expected three TOML numbers: ") +
                             key);
  }
...
SimulationConfig LoadConfiguration(const std::filesystem::path &file) {
  ...
  const auto resolve = [&file](const std::string &value) {
    if (value.empty())
      throw std::runtime_error("Empty configuration path");
  ...
  const auto progressInterval =
      Required<std::int64_t>(document, "runtime.progress_interval");
  ...
  if (progressInterval < 1 || progressInterval > 1000000000) {
    throw std::runtime_error(
        "runtime.progress_interval must be between 1 and 1000000000");
  }
  ...
  if (!std::isfinite(config.transitionEnergyMeV) ||
      !std::isfinite(config.x17MassMeV) ||
      !std::isfinite(config.gunEnergyMeV) || config.transitionEnergyMeV <= 0 ||
      config.x17MassMeV <= 0 || config.gunEnergyMeV < 0) {
    throw std::runtime_error(
        "Source energies and mass must be finite and valid");
  }
  ...
  if (!std::isfinite(optics.emissionMinEv) ||
      !std::isfinite(optics.emissionMaxEv) ||
      !std::isfinite(optics.yieldPerMeV) ||
      !std::isfinite(optics.resolutionScale) ||
      !std::isfinite(optics.decayTimeNs) ||
      !std::isfinite(optics.scintillatorIndex) ||
      !std::isfinite(optics.scintillatorAbsorptionCm) ||
      !std::isfinite(optics.airIndex) ||
      !std::isfinite(optics.quantumEfficiency) || optics.emissionMinEv <= 0 ||
      optics.emissionMaxEv <= optics.emissionMinEv || optics.yieldPerMeV <= 0 ||
      optics.resolutionScale <= 0 || optics.decayTimeNs <= 0 ||
      optics.scintillatorIndex <= 0 || optics.scintillatorAbsorptionCm <= 0 ||
      optics.airIndex <= 0 || optics.quantumEfficiency <= 0 ||
      optics.quantumEfficiency > 1) {
